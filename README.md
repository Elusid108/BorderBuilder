...
**v0.6.1** — Hexagon / Octagon / Rectangle / Circle outers around an imported lithophane no longer warp to the inner teeth; moulding Width / Height commit on blur or Enter; import, shape, and profile changes show a live busy phase until the mesh is ready. **v0.6.0** — Back rabbet is a sharp parallel of the LithoLab pack plus Fit (default 0.8 mm). After import, Rectangle / Hexagon / Octagon / Circle set a user-sized outer while the lithophane hole and rabbet stay. v0.5.1 pinned the front to the unsmoothed plate. v0.5.0 followed the pack on the back but lofted a smoothed sight on the front. Square is gone; Hexagon, Octagon, and Circle size by circumradius. v0.4.2 tried to follow a smoothed sight offset and still sat on the inner magenta. v0.4.1 — Letter-A and other notched organic frames: the moulding follows the traced outline (including the gap between an A’s legs) instead of a convex hull or a straight bar across the bottom; the preview shows the masked letter, not the unclipped photo; offset-loft rings stay corresponded to the sight so the legs no longer fold through the opening. v0.4.0 classified stencil holes and kept gray mask fill inside the silhouette. The original “Phase 1 rectangles only” plan has been overtaken by LithoLab import and mixed plan shapes. This README describes what actually ships, then a tentative backlog.
...
- **LithoLab import:** drop an STL zip or `.litholab` file. The **unsmoothed mask / plate** is the front opening; export width/height set dest/sight. The **back well** is a **miter parallel** of LithoLab’s pack outline (mask disk-offset by `export.border`) plus **Fit clearance** (default **0.8 mm**, minimum 0.8 mm on import). After import, Rectangle / Hexagon / Octagon / Circle keep that opening and rabbet and become the **outer** edge (you set outer width/height or circumradius). The pack-name shape still follows the lithophane outline. Rabbet depth is lithophane stack height plus **0.4 mm** slack. Changing Fit clearance rebuilds the pocket without re-importing. Preview artwork is `original-masked.png` composited to the letter body; enclosed **counters** stay clear (not filled from the raw photo, not painted as a floating island). Stencil **holes** are classified and **ignored** for moulding: the frame traces the letter’s outer outline (including the gap between an A’s legs) and does not add an inner wall around the triangle. Mid-gray mask fill stays inside (near-black cutoff), so a gradient in the letter body does not punch the silhouette.
- **Imported plan kernels:**
  ...
  - **Organic silhouettes** (heart, waterdrop, letter, freeform) loft from the **unsmoothed plate** (front `u = 0`) with an **offset loft**. Artwork clipping may still use a routed spline. The **back rabbet wall** is a miter parallel of the pack plus Fit. After import you can replace the decorative outer with a rectangle, hex, octagon, or circle — that outer is independent of the lithophane teeth. The pack-name shape still follows the outline; those rings stay corresponded to the plate so a notch that the outer offset fills (an A’s legs) does not twist or overlap.
- **Sizing:** artwork/sight width × height for rectangles; radius for hex / oct / circle; moulding width; moulding height (overall Z). Outer size is `sight + 2 × moulding width` for presets without an import. After import, the size fields are dest (pack-name shape) or the **outer** size (rectangle / hex / oct / circle).
...
- **Preview:** orbitable Three.js view. Moulding width/height apply on blur or Enter (the profile sketch still updates while you type). Import, shape, and profile changes show a busy overlay with a live phase until the mesh is ready. Faceted shading on polygonal imports; smoother shading on organic ones.
- **Export:** binary STL, one watertight solid, millimetres, header `BorderBuilder v0.6.1`.
