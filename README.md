...
- **Imported plan kernels:**
  ...
  - **Organic silhouettes** (heart, waterdrop, letter, freeform) loft from the **unsmoothed plate** (front `u = 0`) with an **offset loft** (EDT / disk offset). Artwork clipping may still use a routed spline. The decorative outer fills concave clefts instead of self-intersecting. The **back rabbet wall** is a forced ring: pack outline plus Fit clearance. Each ring stays corresponded to the plate so a notch that the outer offset fills (an A’s legs) does not twist or overlap.
...
- **Export:** binary STL, one watertight solid, millimetres, header `BorderBuilder v0.5.1`.
